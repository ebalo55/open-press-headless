...
export class JwtStrategy extends PassportStrategy(Strategy) {
	...
	get onBeforeValidation() {
		...
		return this._events.get(PASSPORT_JWT_STRATEGY_EVENTS.before_validation).asEvent();
	}
	...
	get onValidationSuccess() {
		/* istanbul ignore next */
		return this._events.get(PASSPORT_JWT_STRATEGY_EVENTS.validation_success).asEvent();
	}
	...
	get onValidationFailed() {
		/* istanbul ignore next */
		return this._events.get(PASSPORT_JWT_STRATEGY_EVENTS.validation_failed).asEvent();
	}
	...
	async validate(payload: JwtPayload): Promise<UserDocument | null> {
		this._events.get(PASSPORT_JWT_STRATEGY_EVENTS.before_validation).dispatch(this, { payload });

		if (payload.sub) {
			return tap(await this._user_service.find(payload.sub), (user) =>
				this._events.get(PASSPORT_JWT_STRATEGY_EVENTS.validation_success).dispatch(this, { user })
			);
		}

		this._events.get(PASSPORT_JWT_STRATEGY_EVENTS.validation_failed).dispatch(this, { payload });
