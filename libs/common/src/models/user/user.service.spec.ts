...
describe("UserService", () => {
	...
	const matchesUserProperties = (value: any, properties: object) => {
		for (const [key, val] of Object.entries(properties)) {
			expect(value[key]).toEqual(val);
		}
	};

	const matchesPassword = async (value: any, password: string) => {
	...
	it("cannot create user with existing email", async () => {
		...
		try {
			...
			expect(VerificationEmailSentErrorFactory.is(error)).toBeTruthy();
		}
	});
	...
	it("can update user by id", async () => {
		...
		const value = await service.update(original_user.id, {
			name: "test",
		});
	...
	it("can update user by instance", async () => {
		...
		const value = await service.update(original_user, {
			name: "test",
		});
	...
	it("cannot update non existing user", async () => {
		try {
			...
			expect(UserNotFoundErrorFactory.is(error)).toBeTruthy();
		}
	});
	...
	it("cannot update user email if email is already used", async () => {
		...
		try {
			await service.update(original_user, {
				email: "test@example.com",
			});
	...
	it("can find all users", async () => {
		...
		expect(value[0].email).toEqual(user_basic_properties.email);
		expect(value[1].email).toEqual(email1);
		expect(value[2].email).toEqual(email2);
	});
