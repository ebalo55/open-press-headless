...
export class HashService {
	constructor(@Inject(AUTH_CONFIG_KEY) private readonly auth_config: AuthConfig) {}
	...
	public async make(password: string): Promise<string> {
		return bcrypt.hash(
			password,
			await bcrypt.genSalt(this.auth_config.hashing.iterations, this.auth_config.hashing.version)
		);
	}
